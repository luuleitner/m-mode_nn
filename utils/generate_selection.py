...
def find_experiments(raw_data_path):
    ...
    Returns:
        List of dicts with session, experiment, path info (sorted by session, then experiment number)
    """
    experiments = []

    # Find all experiment folders (session*/N where N is a number)
    pattern = os.path.join(raw_data_path, "session*", "*")
    for exp_path in glob.glob(pattern):
        if os.path.isdir(exp_path) and os.path.basename(exp_path).isdigit():
            session_name = os.path.basename(os.path.dirname(exp_path))
            exp_num = int(os.path.basename(exp_path))
    ...
    # Sort by session name, then by experiment number (numeric)
    experiments.sort(key=lambda x: (x['session'], x['experiment']))
...
def generate_csv(experiments, output_path, include_path=False):
    ...
    """
    fieldnames = ['session', 'experiment', 'include']
    if include_path:
    ...
    print(f"  Sessions: {len(set(e['session'] for e in experiments))}")
...
def main():
    ...
    parser.add_argument('--data-path', '-d',
                       default='/vol/data/2025_wristus_wiicontroller_leitner/raw',
                       help='Path to raw data directory')
    parser.add_argument('--output', '-o',
                       default='config/preprocessing_selection.csv',
